 ...
 *
 * Runs fully isolated: no MCP servers, no tools, and an empty temp cwd so the
 * child never auto-loads this repo's CLAUDE.md / settings. All untrusted content
 * (case data + the user's free-text question) goes in on stdin.
 *
 * The spawn machinery and the INJECTION BOUNDARY that governs it now live in
 * ./shared/claudeRun.ts.
 */
...
function formatCase(c: AnalyzableCase, i: number): string {
  ...
  if (tl.length) {
    ...
    lines.push(
      `Timeline (${tl.length} entr${tl.length === 1 ? "y" : "ies"}${omitted > 0 ? `, showing last ${shown.length}` : ""}):`
    );
...
export function buildContext(cases: AnalyzableCase[]): BuildContextResult {
  ...
  const header =
    ...
    (truncatedCases > 0
      ? ` (of ${cases.length}; ${truncatedCases} omitted to keep the analysis focused)`
      : "") +
    "\n";
  return { text: header + "\n" + blocks.join("\n\n"), truncatedCases };
}
...
export interface AnalyzeCallbacks {
  ...
  onDone: (meta: RunMeta & { truncatedCases: number }) => void;
